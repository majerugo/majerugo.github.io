---
# the default layout is 'page'
layout: page
icon: fas fa-info-circle
order: 2
title: Whoami
---

## About Me

Fifth-year student at **EPITA** specializing in **Systems, Networks, and Security (SRS)**. Passionate about offensive security with a strong focus on **penetration testing** and **red teaming**.

I have been practicing cybersecurity since **July 2020** on Root-Me, with intensive focus starting in **April 2025**. My background includes development experience since middle school and hands-on forensics work as an **IT expert assistant**.

Currently seeking opportunities in **penetration testing** and **red team operations** following my upcoming internship focused on ransomware recovery infrastructure.

### Areas of Expertise

- **Binary Exploitation (Pwn)**: Memory corruption, exploit development
- **Active Directory**: Attack paths and privilege escalation
- **Web Application Security**: Vulnerability assessment and exploitation
- **Digital Forensics**: Incident response and evidence analysis

### Technical Skills

**Programming**: Python, C, C++, x86 Assembly, Bash, PowerShell

**Current Goal**: Obtain the **HTB Certified Penetration Testing Specialist (CPTS)** certification

---

## Professional Experience

### IT Expert Assistant
**Digital Forensics Internship**

Extracted usable digital evidence from information systems for judicial investigations.
Automated and optimized expert processes using AI techniques and scripting tools.

---

## Challenges

Practicing offensive security skills on various platforms. Active since **July 2020** with a focus on pwn, web exploitation, and forensics challenges.

Regular writeups are published for retired **Hack The Box** machines.

![Root-me-stats](https://root-me-diff.vercel.app/rm-gh?nickname=Ugo-469234&gstats=show&style=dark){: width="400" height="600" }
_Root-Me Profile_
![Hack-The-Box-stats](https://htb-stats.vercel.app/api/htb.js?id=2786020){: width="600" height="400" }
_Hack The Box Profile_

List of challenge platforms and profiles:

| Platform | Profile |
|----------|---------|
| [Root-Me](https://www.root-me.org/) | [Ugo](https://www.root-me.org/Ugo-469234) |
| [Hack The Box](https://www.hackthebox.com/) | [Ugo321](https://app.hackthebox.com/users/2786020) |

---

## CTF Competitions

Active member of **[HDFR](https://ctftime.org/team/384849)**, ranked **Top 3 in France** (as of December 2025).

Participating in CTF competitions regularly with a focus on pwn exploitation, web exploitation and attack-defense scenarios.

| CTF | Year | Ranking | Categories |
|-----|------|---------|------------|
| [ENOWARS 9](https://9.enowars.com/) | 2025 | 36/111 | Attack-Defense |
| [Enedis CTF](https://ctf.hackthebox.com/event/details/ctf-inter-ecoles-enedis-2974) | 2025 | 10/33 | Web |

---

## Bug Bounty

Responsible disclosure and vulnerability research on bug bounty platforms, including active participation in various programs.


| Date        | Event                  | Link                                                               | Ranking   |
| ----------- | ---------------------- | ------------------------------------------------------------------ | --------- |
| Oct-2025    | Hack4Value Bug Bounty  | [Hack4Value](https://hack4values.eu/)                                            | *N/A*     |

I have create an account **YesWeHack**, but I have not yet participated in any bug bounty programs on this platform.

[YesWeHack account](https://yeswehack.com/hunters/ugomajer123)

---

## Labs
Hands-on lab environments exploring complete attack paths from reconnaissance to post-exploitation.

| Lab | Platform | Focus Area | Status |
|-----|----------|------------|--------|
| *Coming soon* | - | - | 🔄 |

Waiting for new materials to run [GOAD](https://github.com/Orange-Cyberdefense/GOAD) labs.

Each lab includes:
* **Reconnaissance**: Enumeration and information gathering
* **Exploitation**: Vulnerability discovery and exploitation
* **Post-Exploitation**: Privilege escalation, lateral movement, and persistence
* **Lessons Learned**: Key takeaways, methodologies, and skills developed

---

## Sources

Additional source code and references for projects I use that are **not necessarily directly related to offensive security**.

### Security resources

- **[HackTricks](https://book.hacktricks.wiki/fr/index.html)** : A comprehensive collection of hacking techniques and tricks, available in both English and French.
- **[PayloadsAllTheThings](https://swisskyrepo.github.io/PayloadsAllTheThings/)** : A repository of useful payloads and bypasses for various web security vulnerabilities.

### Challenge Stats

- **[root-me-stats](https://github.com/dz-root/Rootme-readme-stats)** : A repository providing a clean interface to track your progress on the Root-Me.org platform.
- **[HackTheBox-stats](https://github.com/dogujen/htb-stats)** : A repository offering a nice interface to visualize your progress on the HackTheBox platform.

## Miscellaneous Projects

- **[Fontawesome Icon](https://fontawesome.com/v5/search)** : A simple tool to search for Fontawesome icons by name or keyword.

---

## Contact

- **GitHub**: [@majerugo](https://github.com/majerugo/)
- **LinkedIn**: [Ugo Majer](https://www.linkedin.com/in/ugo-majer-9331a02b6/)

---

## Disclaimer

All content in this repository is for **educational purposes only**. Techniques and tools documented here should only be used in authorized environments. Always obtain proper permission before testing any system.
